...
task.spawn(function() -- delay execution
    local Players = game:GetService("Players")
    local UserInputService = game:GetService("UserInputService")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")

    local player = Players.LocalPlayer
    if not player then repeat task.wait() until Players.LocalPlayer end
    ...
    local success = pcall(function() gui.Parent = player:WaitForChild("PlayerGui") end)
    if not success then gui.Parent = game:GetService("CoreGui") end
    ...
    attackBtn.MouseButton1Click:Connect(function()
        ...
        attackBtn.Text = "Attack: " .. (autoPower and "ON" or "OFF")
        if autoPower then
            task.spawn(function()
                while autoPower do
                    r:FireServer({})
    ...
    afkBtn.MouseButton1Click:Connect(function()
        antiAFK = not antiAFK
        afkBtn.Text = "AFK: " .. (antiAFK and "ON" or "OFF")
        if antiAFK then
            task.spawn(function()
                while antiAFK do
